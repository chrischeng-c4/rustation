...
**Napi-rs Export Design** (KB-First approach):
1. Define command interface in `kb/architecture/00-overview.md`
2. Implement in `packages/core/src/`
3. Export via `#[napi]`
4. Frontend invokes via `window.api.commandName(params)`
...
<rule severity="NEVER">Mutate state directly from Frontend → Split brain state → Use IPC/Napi to mutate Backend</rule>
...
Task: Add send_prompt Napi Export
File: packages/core/src/workflow.rs
</marker>
