...
fn test_cli_load_state_invalid_json() {
    ...
    assert!(!output.status.success(), "Command should fail for invalid JSON");
}
...
fn test_cli_state_restoration_preserves_all_fields() {
    ...
    assert_eq!(original_state, loaded_state, "State should be preserved exactly");
}
...
fn test_cli_load_state_version_mismatch_warning() {
    ...
    assert!(output.status.success(), "Should succeed with warning in Phase 2");
